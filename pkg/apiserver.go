...
type APIService struct {
	//controllerRegisty is where all registryed controllers exist
	controllerRegistry map[sting]interface{}
...
func (api *APIService) Delete(path, controllerWithActionsString string) {
	mapping := api.mappingRequestMethodWithControllerAndActions(DELETEMethod, path, controllerWithActionString)
	api.registeredPathAndController[path] = mapping
}

// mappingRequestMethodWithControllerAndActions is a function
// for mapping request method with controllers which containing actions
func (api *APIService) mappingRequestMethodWithControllerAndActions(requestMethod, path, controllerWithActionString) {
...
func (api *APIService) HandleRequest(controllers map[string]map[string]string) http.HandlerFunc {
	return func(rw http.ResponseWriter, request *http.Request) {
		...
		matchedControllers, ok := controllers[method]
		if !ok {
			rw.WriteHeader(HTTPMethodNotAllowed)
		}
		for k, v := range matchedControllers {
			controllerKey := "*" + k
...
// Prepare is a function prepare the service and return prefared service to the user
